...
window.BISO.mainTable = (function () {
  const { normalize, findColumn } = window.BISO.utils;
  const {
    mainTableContainer,
    indicatorSelect,
    indicatorPrevBtn,
  ...
  const { renderTable } = window.BISO.tableRender;
  ...
  // first/last entry.
  function stepIndicator(delta, afterSelect) {
  ...
  // from app.js, which composes stats-chart.js's functions) invoked once the
  // indicator's own re-render is done — errors from it are reported the same way
  // as everywhere else in the widget.
  function selectIndicator(value, afterSelect) {
